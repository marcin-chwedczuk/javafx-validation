...
class TypeRef {
    ...
    public boolean matches(Class<?> type) {
        if (type.getTypeParameters().length != this.typeArguments.size()) {
            // Different number of type parameters
            return false;
        }
        else if (!this.baseType.equals(type)) {
            // Different base type
            return false;
        }
        else {
            TypeVariable<? extends Class<?>>[] params = type.getTypeParameters();
            Map<String, Class<?>> subst = new HashMap<>();
